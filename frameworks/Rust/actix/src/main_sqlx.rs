...
async fn world_row(db: web::Data<Addr<db_sqlx::DbExecutor>>) -> Result<HttpResponse, Error> {
    let res = db
        .send(db_sqlx::RandomWorld)
        .await
...
async fn queries(
    req: HttpRequest,
    db: web::Data<Addr<db_sqlx::DbExecutor>>,
...
async fn fortune(db: web::Data<Addr<db_sqlx::DbExecutor>>) -> Result<HttpResponse, Error> {
    let res = db
        .send(db_sqlx::TellFortune)
        .await
        .map_err(|e| ErrorInternalServerError(e))?;
    match res {
        Ok(rows) => {
...
async fn main() -> std::io::Result<()> {
    ...
    let addr =
        SyncArbiter::start(num_cpus::get() * 3, move || db_sqlx::DbExecutor::new(db_url));
